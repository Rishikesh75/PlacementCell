...
export class FeedbackOnCompanyMapper {
  ...
  private mapToTechnicalRound(apiResponse: FeedbackOnCompanyResponseDto): TechnicalRound {
    ...
    const CoreCSQuestions: ComputerCoreQuestion[] = (techInfo.CoreCSQuestions || []).map(q => 
  ...
  mapToApiRequest(feedback: Feedback): FeedbackOnCompanyResponseDto {
    return {
      Id: 'fb124', // This will be generated by the backend
      CompanyId: 'comp123', // This will be set by the backend or from user input
      company: null,
      Id: 'alum123', // This will be set by the backend based on auth
      alumni: null,
      ...
      technicalRoundInfo: {
        ...
        CoreCSQuestions: feedback.technicalRound.computerCoreQuestions.map(q => ({
          questionType: q.questionType,
          question: q.question
        })),
