...
public class TeacherService {
    ...
    public UUID findIdByEmail(String email, UUID collegeId, String password) {
        Teacher teacher = repository.findByCollegeIdAndEmailIgnoreCase(collegeId, email.trim())
                .orElseThrow(() -> new ResourceNotFoundException(
                        "Teacher not found for email: " + email
                ));

        UserAccount account = userAccountRepository.findByTeacherId(teacher.getId())
                .orElseThrow(() -> new ResourceNotFoundException(
                        "Teacher account not found for email: " + email
                ));
